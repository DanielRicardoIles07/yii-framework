...
class SiteController extends Controller
{
 ...
 public function actionDownload()
 {
  if (Yii::$app->request->get("file"))
  {
   ...
   if (!$this->downloadFile("archivos/", Html::encode($_GET["file"]), ["pdf", "txt", "doc","jpg", "png", "gif"]))
   {
    ...
    public function actionUpload()
 {
  ...
  if ($model->load(Yii::$app->request->post()))
  {
   ...
   #para un input file simple, subir un solo archivo
   //$model->file = UploadFile::getInstance($model, 'file');
   //$file = $model->file;
   //$file->saveAs('archivos/' . $file->baseName . '.' . $file->extension);
   //$msg = "<p><strong class='label label-info'>Excelente, archivos subidos con éxito</strong></p>";

   if ($model->file && $model->validate()) {
    foreach ($model->file as $file) {
     $file->saveAs('archivos/' . $file->baseName . '.' . $file->extension);
     $msg = "<p><strong class='label label-success'>Excelente, archivos subidos con éxito</strong></p>";
    }
   }
  }
  ...
  if ($model->load(Yii::$app->request->post()))
  {
   if ($model->validate())
   {
    ...
    if ($table->count() == 1)
    {
     ...
     $body .= "<p><a href='http://localhost/2.0.12/web/index.php?r=site/resetpass'>Recuperar Contraseña</a></p>";
   ...
   if ($model->validate())
   {
    ...
    if ($recover == $model->recover)
    {
     ...
     if ($table->save())
     {
      ...
      $msg = "Enhorabuena, Contraseña reestablecida correctamente, redireccionando a la página de login ...";
      $msg .= "<meta http-equiv='refresh' content='1; ".Url::toRoute("site/login")."'>";
     }
     else
     {
      $msg = "Ha ocurrido un error";
        ...
        if ((int) $id)
        {
            ...
            if ($model->count() == 1)
            {
                ...
                if ($activar->update())
                {
                    ...
                    echo "<meta http-equiv='refresh' content='0.2; ".Url::toRoute("site/login")."'>";
                }
                else
                {
  ...
  if ($model->load(Yii::$app->request->post()))
  {
   if($model->validate())
   {
    ...
    if ($table->insert())
    {
     ...
     $body .= "<a href='http://localhost/2.0.12/web/index.php?r=site/confirm&id=".$id."&authKey=".$authKey."' '>Confirmar</a>";
  ...
  return $this->render("register", ["model" => $model, "msg" => $msg]);
    ...
    public function actionDelete()
    {
        if(Yii::$app->request->post())
        {
            ...
            {
                echo "Ha ocurrido un error al eliminar el alumno, redireccionando ...";
                echo "<meta http-equiv='refresh' content='10; ".Url::toRoute("site/view")."'>";
            }
        }
        else
        {
   ...
   public function actionView()
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post())) 
        {
            if ($model->validate()) {
                ...
                if ($table->insert()) 
                {
                    $msg = "<p style='color: green'>Los datos se han registrado con exito <i style='font-size:24px' class='fa'>&#xf058;</i></p>";
                    $model->nombre = null;
                ...
                {
                    $msg = "<p style='color: red'>Ha ocurrido un error, no se pueden registrar <i style='font-size:24px' class='fa'>&#xf00d;</i></p>";
                }
    ...
    public function actionRequest()
    {
        ...
        {
            $mensaje = "Envio resulto exitoso, has enviado: " ."<b>". $_REQUEST["nombre"]."</b>";
        }
    ...
    public function actionValidarformularioajax()
    {
        ...
        if ($model->load(Yii::$app->request->post())) 
        {
            ...
            {
                # hacer consulta a bd
                //$msg = "enviando correctamente";
                $msg = "Envio resulto exitoso, has enviado estos datos : <br>Nombre: " ."<b>". $model->nombre."</b><br>Email: "."<b>". $model->email."</b>";
                $model->nombre = null;
                $model->email = null;
            }
            else
